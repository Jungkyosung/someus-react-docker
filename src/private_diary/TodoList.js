...
const TodoList = (props) => {
    ...
    const handlerInputFocusOut = async (id, e) => {

        //Todos 내용 중에 받아온 id와 일치하는 goal객체를 수정 반영
        let updateArray = [...props.todos];
        const index = updateArray.findIndex(item => item.goalId === id);
        updateArray[index] = { ...updateArray[index], goalContents: e };

        //인자 id를 가지고 해당 goalId를 갖고 하나의 객체를 가져와서 할당
        const arrayTodos = updateArray.find(array => {
    ...
    // [++수정후 핸들러++]{ 핸들러 } 하트 그림 클릭 -> [빈하트, 반하트, 꽉찬 하트] 로 변경 (작업완료)
    const handlerImgClick = async (id) => {

        //인덱스를 구하고, 해당 goal의  goalState를 0, 1, 2가 반복 수정되도록 함 
        // await props.setTodos(prevState => {
        //     const updateArray = [...prevState];
        //     const index = updateArray.findIndex(item => item.goalId === id);
        ...
        //setTodos를 사용해야 한다는 생각 때문에 이상하게 코딩 했었음. 값을 가지고 노는 건 앞에서 하고 출력할 때,
        //setTodos를 해서 화면에 보이게만 해주면 문제가 없음. setTodos의 위치가 중요포인트!!!
        //나는 async await 같은 비동기를 동기처리로 바꿔주는 게 필요한 줄 알았는데 아니었음.
